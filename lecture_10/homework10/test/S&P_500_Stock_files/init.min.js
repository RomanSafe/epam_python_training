...
* automatic comment
*/;
var config={apiKey:"AIzaSyA0Pztlypm8w6eE1TYQDbMDmuM0UL2c21U",authDomain:"marketsinsidercom.firebaseapp.com",databaseURL:"https://marketsinsidercom.firebaseio.com",projectId:"marketsinsidercom",storageBucket:"marketsinsidercom.appspot.com",messagingSenderId:"608989885452"};firebase.initializeApp(config);var firebaseAuthorizationKey="AAAAjcqgNAw:APA91bEHDaltl1Mm65h0YVyxOUlAUCAEXnVzV8EXHL-etIGXMqHofOwLazzHBEdFYQ5HI0p822fH5F014uh6hBVWLVRpy_BtvTVaMVIM7G5g-JhJ8C8f1aRv9ie18xzpgewEuVXbSGfL"

/**
* automatic comment
*/;
var logger=new WebPushLogger(webPushSettings);var webPush=new WebPush(webPushSettings,logger);var fcmWebPush=null;var wnsWebPush=null;if(window.location.pathname==='/webpushtest'){function finWebpushInit(requestPermission){try{if('serviceWorker'in navigator){webPush.registerServiceWorker().then(function(registration){logger.log("ServiceWorker successfully registered!","colorBlack");if(typeof requestPermission!='boolean'||requestPermission===true){fcmWebPush=new FcmWebPush(registration,webPush._settings,logger);fcmWebPush.init();wnsWebPush=new WnsWebPush(registration,webPush._settings,logger);wnsWebPush.init()}}).catch(function(e){logger.err("Registering ServiceWorker failed: "+e.message,e);return false})}else{logger.log("Service workers are not supported!","colorRed");return false}}catch(e){return false}return true}}else{try{if('serviceWorker'in navigator){webPush.registerServiceWorker().then(function(registration){logger.log("ServiceWorker successfully registered!","colorBlack");fcmWebPush=new FcmWebPush(registration,webPush._settings,logger);fcmWebPush.init();wnsWebPush=new WnsWebPush(registration,webPush._settings,logger);wnsWebPush.init()}).catch(function(e){logger.err("Registering ServiceWorker failed: "+e.message,"colorRed")})}else{logger.log("Service workers are not supported!","colorRed")}}catch(e){}}
